            ...
            <div className={s.galleryWrapper}>
                ...
                <div className={s.gallery}>
                    {product.gallery ? product.gallery.slice(0,3).map((img, i)=>(
                        <div className={clsx(s.galleryImage, (currentImage === i) && s.active)} key={img} onClick={()=>{setCurrentImage(i)}}>
                    ...
                    <div className={s.arrows}>
                        <ButtonRound onClick={()=> setCurrentImage(prev => prev - 1 < 0 ? product.gallery.length - 1 : prev - 1)}>
                            <Arrow className={s.prev}/>
                        </ButtonRound>
                        <ButtonRound onClick={()=>setCurrentImage(prev => prev + 1 >= product.gallery.length ? 0 : prev + 1)}>
                            <Arrow/>
                        </ButtonRound>
                    </div>
                    {product.gallery && product.gallery.slice(3).map((img, i)=>(
                        <div className={clsx(s.galleryImage, (currentImage === i+3) && s.active)} key={img} onClick={()=>{setCurrentImage(i+3)}}>
                            <CustomImage src={img} alt={''} fill/>
                        </div>
                    ))}
                </div>
